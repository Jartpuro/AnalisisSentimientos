...
clase_minoritarias = datos_entrenamiento['overall_sentiment'].value_counts().idxmin()
min_samples = datos_entrenamiento['overall_sentiment'].value_counts().min()

# Submuestrear la clase mayoritaria y sobremuestrear las clases minoritarias
data_balanced = pd.concat([
    df.sample(min_samples, random_state=42, replace=len(df) < min_samples)
    for _, df in datos_entrenamiento.groupby('overall_sentiment')
])

print("\nDistribución después del balanceo:")
print(data_balanced['overall_sentiment'].value_counts())

# Visualizar distribución después del balanceo
plt.figure(figsize=(8, 6))
ax = data_balanced['overall_sentiment'].value_counts().plot(
    kind='bar', color=sns.color_palette("Blues", n_colors=3), title="Distribución Equitativa de Clases"
)
plt.xlabel("Clases")
plt.ylabel("Cantidad")
...
plt.savefig(os.path.join(model_base_path, "distribucion_equitativa_clases.png"), dpi=300)
plt.close()

# Convertir texto a TF-IDF
vectorizer = TfidfVectorizer(max_features=1000, ngram_range=(1, 2))
X_train = vectorizer.fit_transform(data_balanced["Texto"])
y_train = data_balanced["overall_sentiment"]
...
metricas_balanceo_equitativo, etiquetas_balanceo_equitativo, resumen_mejor_modelo = entrenar_y_registrar(
    X_train, y_train, X_test, "balanceo_equitativo", param_grid)

# Resumen en Consola
print("\nResumen del mejor modelo con balanceo equitativo:")
