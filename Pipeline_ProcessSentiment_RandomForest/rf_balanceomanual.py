...
print(datos_entrenamiento['overall_sentiment'].value_counts())

# Sobremuestreo de clases minoritarias
data_majority = datos_entrenamiento[datos_entrenamiento['overall_sentiment'] == datos_entrenamiento['overall_sentiment'].value_counts().idxmax()]
data_minority = datos_entrenamiento[datos_entrenamiento['overall_sentiment'] != datos_entrenamiento['overall_sentiment'].value_counts().idxmax()]
data_minority_oversampled = data_minority.sample(len(data_majority), replace=True, random_state=42)

# Combinar las clases balanceadas
datos_entrenamiento_balanceado = pd.concat([data_majority, data_minority_oversampled])

print("\nDistribución después del balanceo:")
print(datos_entrenamiento_balanceado['overall_sentiment'].value_counts())

# Visualizar distribución después del balanceo
plt.figure(figsize=(8, 6))
ax = datos_entrenamiento_balanceado['overall_sentiment'].value_counts().plot(
    kind='bar', color=sns.color_palette("Blues", n_colors=3), title="Distribución Balanceada de Clases"
)
plt.xlabel("Clases")
plt.ylabel("Cantidad")
...
plt.savefig(os.path.join(model_base_path, "distribucion_balanceada_clases.png"), dpi=300)
plt.close()

# Convertir texto a TF-IDF
vectorizer = TfidfVectorizer(max_features=1000, ngram_range=(1, 2))
X_train = vectorizer.fit_transform(datos_entrenamiento_balanceado["Texto"])
y_train = datos_entrenamiento_balanceado["overall_sentiment"]
...
metricas_balanceo_manual, etiquetas_balanceo_manual, resumen_mejor_modelo = entrenar_y_registrar(X_train, y_train, X_test, "balanceo_manual", param_grid)

# Resumen en Consola
print("\nResumen del mejor modelo con balanceo manual:")
