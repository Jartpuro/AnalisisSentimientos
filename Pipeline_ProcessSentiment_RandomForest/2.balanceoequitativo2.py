...
# ----------------------------
transcripciones_path = r"C:\Users\Andrea\OneDrive\Documentos\1.Maestria\ProyectoGrado\audios_txt"  # Ruta a la carpeta de transcripciones
muestra_path = r"C:\Users\Andrea\OneDrive\Documentos\1.Maestria\ProyectoGrado\MuestraEntrenamiento.xlsx"  # Ruta a la muestra de archivos
etiquetas_reales_path = r"C:\Users\Andrea\OneDrive\Documentos\1.Maestria\ProyectoGrado\Dataset_Global_Sentiment.xlsx"  # Ruta al archivo de etiquetas reales
...
# Visualizar la distribución de clases en los 100 datos etiquetados (sin balancear)
...
# ==========================================
# BALANCEO EQUITATIVO DE CLASES
# ==========================================
print("\nDistribución antes del balanceo:")
print(labeled_data['overall_sentiment'].value_counts())

# Se determina el número máximo de muestras entre todas las clases
max_count = labeled_data['overall_sentiment'].value_counts().max()

# Para cada clase, se muestrea (con reemplazo) hasta alcanzar max_count
dfs = []
for clase in labeled_data['overall_sentiment'].unique():
    df_clase = labeled_data[labeled_data['overall_sentiment'] == clase]
    df_clase_balanced = df_clase.sample(n=max_count, replace=True, random_state=42)
    dfs.append(df_clase_balanced)
labeled_data_balanced = pd.concat(dfs)
print("\nDistribución después del balanceo equitativo:")
print(labeled_data_balanced['overall_sentiment'].value_counts())

# Graficar la distribución resultante del balanceo equitativo
fig = plt.figure(figsize=(8, 6))
ax = labeled_data_balanced['overall_sentiment'].value_counts().plot(
    kind='bar', color=sns.color_palette("Blues", n_colors=3),
    title="Distribución de Clases (Balanceo Equitativo)"
)
ax.set_xlabel("Clases")
ax.set_ylabel("Cantidad")
ax.set_xticklabels(ax.get_xticklabels(), rotation=0)
for p in ax.patches:
    ax.annotate(str(p.get_height()), (p.get_x() + p.get_width()/2., p.get_height()),
                ha='center', va='baseline')
guardar_grafico(fig, "distribucion_clasesetiq_balanceoequitativo.png")

# ==========================================
# CONVERSIÓN A TF-IDF (USANDO LOS 100 DATOS ETIQUETADOS BALANCEADOS)
# ==========================================
...
# ==========================================
# EVALUACIÓN DEL MODELO SOBRE LAS 100 MUESTRAS (Validación Cruzada)
# ==========================================
# Utilizamos cross_val_predict para obtener predicciones para cada una de las 100 muestras
...
plt.title("Matriz de Confusión - 100 Datos Etiquetados (Balanceo Equitativo, Validación Cruzada)")
plt.xlabel("Predicción")
plt.ylabel("Real")
guardar_grafico(fig, "matriz_confusion_cv_100etiq_balanceoequitativo.png")
...
print("Métricas en el conjunto de 100 datos etiquetados (validación cruzada):")
...
archivo_predicciones = os.path.join(model_base_path, "predicciones_todo_balanceoequitativo.csv")
